...
2. 扫描 `modules / tools / hooks / commands`
...
在这个框架里，CLI 不是单独写成 `cli.ts`，而是通过 `defineCommand()` 定义，再自动桥接到 OpenClaw 的 `registerCli()`。

所以：

- 想注册命令，请写 `commands/*.command.ts`
- 不需要自己直接调用 `api.registerCli()`
...
如果你什么都没读过，只照着这 7 步做，也能得到一个可运行插件。
...
### 第 6 步：写一个 CLI `command`

创建 `src/my-plugin/commands/status.command.ts`：
...
export default defineCommand({
  ...
  description: "Print plugin runtime diagnostics.",
...
- 你写的是 `command`
- 框架内部会把它自动桥接成 OpenClaw CLI
- 最终宿主层用的是 `registerCli()`

所以，`command` 就是本框架里注册 CLI 的标准方式。

### 第 7 步：写插件入口 `index.ts`
...
1. 读取 manifest
2. 读取生成后的 registry
3. 导出一个 OpenClaw 能识别的 `register(api)` 入口
...
3. **扫描** `modules/*.module.ts`、`tools/*.tool.ts`、`hooks/*.hook.ts`、`commands/*.command.ts`
...
| `*.command.ts` | `execute(args, context)` | `api.registerCli(({ program }) => ...)` |
...
console.log(await host.runCommand("framework:status", ["--verbose"]));
```
...
### 4. 为什么 CLI 不是写成 `cli.ts`

因为在这个框架里：

- `CLI = command 定义 + OpenClaw 适配器桥接`

这是框架的设计约定，不是缺功能。

### 5. `command` 的参数为什么是 `string[]`
...
10. 本地调用 `tool / hook / command` 至少验证过一次
